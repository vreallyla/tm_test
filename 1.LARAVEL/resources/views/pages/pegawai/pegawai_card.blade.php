<article class="bg-card rounded-lg h-[17.5rem]" x-data="{{ json_encode($dt) }}">
    <header class="flex flex-col items-center justify-center h-56">
        <div class="ring-2 ring-badge text-center rounded-full overflow-hidden w-24 mx-auto">
            <img class="object-cover h-24 w-full " src="{{$dt->photo}}">
        </div>
        <h4 class="text-2xl">{{ $dt->nama }}</h4>
        <span class="text-icon">{{ $dt->no_pegawai }}</span>
        <span class="text-icon">{{ $dt->poli }}</span>
    </header>
    <footer class="flex h-12 divide-x-2 divide-badge py-2 px-2">
        <div class="w-1/2 relative">
            <a href="{{ route('pegawai.edit', ['pegawai' => $dt->no_pegawai]) }}"
        ...
        <div class="w-1/2 relative">
            <button @class([
                ...
                @click="if(!is_used){deleteCode=no_pegawai;deleteName=nama}">
