@props(['label', 'required' => false, 'name', 'disabled' => false, 'value' => null, 'type' => 'text'])
<div>
    ...
    <input @disabled($disabled) @if ($required) required @endif type="{{ $type }}"
        name="{{ $name }}" id="{{ $name }}"
        placeholder="{{ $disabled ? '' : 'isi ' }}{{ $label }}..." value="{{ $value }}"
        @if ($type == 'file') accept="image/png, image/jpeg, image/webp" @endif
        @class([
